...
def Analyzer(df):
    ...
    df['Polarity Score']=df["clean_review"].apply(lambda x: SIA.polarity_scores(x)['compound'])
    df['Neutral Score']=df["clean_review"].apply(lambda x: SIA.polarity_scores(x)['neu'])
    df['Negative Score'] = df["clean_review"].apply(lambda x: SIA.polarity_scores(x)['neg'])
    df['Positive Score'] = df["clean_review"].apply(lambda x: SIA.polarity_scores(x)['pos'])
    df['Sentiment']= ''
...
def upload_file():
    ...
    if data_file is not None:
        ...
        with st.expander(" Expand to see data"):
            st.dataframe(df)
        st.success(str(df.shape[0])+' reviews successfully loaded!')
        return df 
...
if __name__ == '__main__':
    ...
    menu_id = option_menu(
            ...
            options=["Home", "Sent Analysis","Results"],  # required
    ...
    if menu_id =='Home':
        ...
        col1.write("") 
        col1.write("")
        col1.write("This is the tool for analyzing the reviews of tours using sentiment analysis techniques and nlp models")
        #col2.image("images/logo.png",use_column_width=True)
        col2.image("images/logo1.png",use_column_width=True)
        ...
        if df is not None:
            ...
            with st.spinner('Wait text processing in progress ..'):
                ...
                df['clean_review'] = df['clean_review'].apply(lambda x: re.sub(r"http\S+","",str(x)))
                ...
                df['clean_review']=df['clean_review'].apply(lambda x:[w for w in x if len(w) >= 3])
                # Stitch tokens back together 
                df['clean_review']=df['clean_review'].apply(lambda x:" ".join(x))
                df = Analyzer(df)
                st.subheader('Before Cleaning vs After Cleaning')  
                st.table(df[['review','clean_review']].head(10))
                st.session_state.datafrm = df[['clean_review','Sentiment']]
                st.subheader('Reviews after sentiment analysis')
                st.dataframe(df[['review','clean_review','Sentiment']])
        ...
        #fig = pp.bar(df,x='Sentiment',y='Counts',color='Sentiment',text="Sentiment",color_discrete_sequence=["#DEB887", "#F5DEB3", "#C8AD7F"],)
        c11,c22=st.columns((3,2))
        #c22.markdown('<br><br>',unsafe_allow_html=True)
        #tweets = tweets[[,'Sentiment','Counts']]
        ...
        with st.spinner('Wait WordCounts processing in progress ..'):
            ...
            words= " ".join(tweet for tweet in dff.clean_review)
